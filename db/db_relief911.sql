...
--
-- Host: localhost
-- Generation Time: May 04, 2015 at 11:33 PM
...
CREATE TABLE IF NOT EXISTS `victim` (
  ...
  `house_conditions` varchar(4000) NOT NULL,
  `family_conditions` varchar(4000) NOT NULL,
  `flag_safe_water` enum('1','0') NOT NULL,
  `flag_safe_food` enum('1','0') NOT NULL,
  `flag_medical_access` enum('1','0') NOT NULL,
  `flag_temporary_shelter` enum('1','0') NOT NULL,
  `flag_permanent_shelter` enum('1','0') NOT NULL,
  `flag_contagious_disease` enum('1','0') NOT NULL,
  `psychological_state` varchar(300) NOT NULL COMMENT 'Use JSON/XML String to store different values',
  `physical_state` varchar(300) NOT NULL COMMENT 'Use JSON/XML String to store different values',
  `victim_condition` varchar(300) NOT NULL,
  `economic_state` varchar(300) NOT NULL,
  `initial_assessment` varchar(4000) NOT NULL,
  `date_added` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `photographs` varchar(100) NOT NULL,
  PRIMARY KEY (`id`)
...
--
-- Table structure for table `victim_family`
--

CREATE TABLE IF NOT EXISTS `victim_family` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `date_added` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `victim_parent_id` (`victim_parent_id`)
